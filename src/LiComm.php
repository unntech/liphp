...
class LiComm {
    ...
    {
        if ( stripos( $_SERVER[ 'HTTP_USER_AGENT' ], 'wxwork' ) !== false ) {
            return 4;
        } elseif ( stripos( $_SERVER[ 'HTTP_USER_AGENT' ], 'MicroMessenger' ) !== false ) {
            return 1;
    ...
    public static function url2https(string $url): string
    {
		//return str_replace("http://","https://",$url);
		return preg_replace('/^http:/i','https:',$url);
	}
    
    public static function is_robot(): bool|int
    {
     ...
     * @return void
     */
    public static function consoleProgressBar(int $total, int $current, int $barLength = 30): void
    {
        $percent = ($current / $total) * 100;
        $filledLength = (int)floor($barLength * $percent / 100);
